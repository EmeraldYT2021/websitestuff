...
- Drag objects to reparent
	...
	local GuiColor = {
		...
		Border          = dark;
		Selected        = inverted;
		BorderSelected  = inverted;
		Text            = BrickColor.Black().Color;
		TextDisabled    = dark;
		TextSelected    = light;
		Button          = light;
		ButtonBorder    = dark;
	...
	script.RemoteEvent.OnServerEvent:Connect(function(User, TextBox, Text)
		if TextBox == explorerFilter then
			explorerFilter.Text = Text
			rawUpdateList()
		end
	end)
...
--]]
	...
	local function Round(number, decimalPlaces)
		return tonumber(string.format("%." .. (decimalPlaces or 2) .. "f", number))
	end
	...
	local function ToString(value, type)
		...
		elseif type == "UDim2" then
			local xScale = Round(value.X.Scale)
			local xOffset = Round(value.X.Offset)
			local yScale = Round(value.Y.Scale)
			local yOffset = Round(value.Y.Offset)
			
			return "{"..tostring(xScale)..", "..tostring(xOffset).."}, {"..tostring(yScale)..", "..tostring(yOffset).."}"
		else
			return tostring(value)
		end
	end
	...
	Controls["default"] = function(object, propertyData, readOnly)
		...
		if not readOnly then
			script.RemoteEvent.OnServerEvent:Connect(function(User, TextBox, Text)
				if TextBox == box then
					Set(object, propertyData, ToValue(Text,propertyType))
					update()
				end
			end)
		end
	...
	Controls["Color3"] = function(object, propertyData, readOnly)
		...
		script.RemoteEvent.OnServerEvent:Connect(function(user, instance, text)
			if instance == textBox then
				Set(object, propertyData, ToValue(text,"Color3"))
				local value = object[propertyName]
	...
	function showProperties(obj:Instance)
		ClearPropertiesList()
		...
		for _,nextObj in pairs(obj) do
			if not foundProps[nextObj.className] then
				...
				local propertiesTableThingIDK = RbxApi.GetProperties(nextObj.className)
				for i,v in pairs(propertiesTableThingIDK) do
